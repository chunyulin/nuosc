...
#!/bin/bash
#SBATCH -J ${runtag}_${jobtag} -t 8:00:0
...
for i in \$(seq 1 \$N); do
  ...
  ./nuosc --np ${px} ${py} ${pz} --pmo 0 --mu 1 --ko 1e-3 --ipt 0   --xmax ${xm} ${ym} ${zm} --dx 0.1 \
          --nv ${nv} --nphi ${nphi}  --cfl 0.5 --alpha 0.9 --eps0 1e-3 --sigma 5.0   --ANA_EVERY 5 --END_STEP 10
...
#!/bin/bash
#SBATCH -J ${runtag}_${jobtag} -t 8:00:0
#SBATCH --nodes=${node} --ntasks-per-node=${gpu} --cpus-per-task=$((10*gpu))
#SBATCH --gres=gpu:${gpu} --mem-bind=no
##export OMP_PROC_BIND=true
##export OMP_PLACES=cores
##export OMP_DYNAMIC=false
VARS="SLURM_JOB_ID SLURM_NTASKS SLURM_NNODES SLURM_NTASKS_PER_NODE SLURM_CPUS_PER_TASK SLURM_TASK_PID SLURM_NODELIST \
...
test_one() {
 ...
 local nv=$1; shift
 local nphi=$1; shift
 sub_t4 ${tag}  $((px*py*pz))    1 80   $px $py $pz  $((b*px)) $((b*py)) $((b*pz))  $nv $nphi
}
...
test_gpu() {
 ...
 local nv=$1; shift
 local nphi=$1; shift
 sub_gpu ${tag} $node $gpu $px $py $pz  $((b*px)) $((b*py)) $((b*pz))  $nv $nphi
}
...
#test_one f2fd 1 1 1   5  14 10
test_gpu acc 1 1   1 1 1   3  12 10
