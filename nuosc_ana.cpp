...
void NuOsc2D::eval_conserved(const FieldVar* __restrict v0) {
...
void NuOsc2D::renormalize(const FieldVar* __restrict v0) {
...
FieldStat NuOsc2D::_analysis_v(const real var[]) {
...
FieldStat NuOsc2D::_analysis_c(const real vr[], const real vi[]) {
...
void NuOsc2D::output_detail(const char* filename) {
...
void NuOsc2D::analysis() {
    ...
    // integral over (vz,z)
    #pragma omp parallel for reduction(+:avgP,avgPb,aM01,aM02,aM03,aM11,aM12,aM13,norP,norPb,nor) reduction(max:maxrelP,maxrelN)  collapse(3)
     FORALL(i,j,v)  {
	int ij = idx(i,j,v);
