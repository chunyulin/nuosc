...
#pragma omp parallel for reduction(+:avgP,avgPb,aM01,aM02,aM03,nor,norb,surv,survb,mm,mmb) reduction(max:maxdP,maxdN) collapse(3)
#pragma acc parallel loop reduction(+:avgP,avgPb,aM01,aM02,aM03,nor,norb,surv,survb,mm,mmb) reduction(max:maxdP,maxdN) collapse(3)
    FORALL(i,j,v)  {
        int ijv = idx(i,j,v);
        ...
        mm  += vw[v]* v_stat->xx [ijv];
        mmb += vw[v]* v_stat->bxx[ijv];
