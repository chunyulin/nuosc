...
class Sphere {
    public:
        ...
        void init_f_s4() {
            ...
            for (uint i=0;i<N; ++i)  f[i] = std::pow(vx[i]*vy[i]*vz[i],4);
...
class SpherePolarVZ : public Sphere {
    public:
        uint nv, np;
        SpherePolarVZ(uint nv_, uint np_) : nv(nv_), np(np_) {
            N = nv*np;
            vx.reserve(N);
            vy.reserve(N);
            vz.reserve(N);
...
int main(int argc, char *argv[]) {
    ...
    for (uint n=2; n<2*N; n*=2) {
       t1 = std::chrono::high_resolution_clock::now();
       SpherePolarVZ sphere(n,n);
       real integral = sphere.integral();
       real stepms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now()-t1).count();
       printf("SphereVZ, n = %5d, I= %15.10g (%7.4f %%)  Time: %g ms\n", n, integral, (sphere.exact- integral)/sphere.exact, stepms);
    }
    ...
    for (uint n=2; n<2*N; n*=2) {
       t1 = std::chrono::high_resolution_clock::now();
       SpherePolarCC sphere(n,n);
       real integral = sphere.integral();
       real stepms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now()-t1).count();
       printf("SphereTP, n = %5d, I= %15.10g (%7.4f %%)  Time: %g ms\n", n, integral, (sphere.exact- integral)/sphere.exact, stepms);
    }
#endif
    ...
    for (uint n=1; n<N; n<<=1) {
       t1 = std::chrono::high_resolution_clock::now();
       IcosahedronDelaunay  sphere(n,n);
       //if (n==1) {
          //sphere.showIcosaExact();
          //sphere.dumpBasePoints();
       //}
       real integral = sphere.integral();
       real stepms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now()-t1).count();
        ...
        for(int i=0;i<sphere.N;++i) {
            max = sphere.vw[i]>max? sphere.vw[i] : max;
            min = sphere.vw[i]<min? sphere.vw[i] : min;
        }

       printf("IcosaDelaunay, n = %5d, I= %15.10g (%7.4f %%),  dVe= %4.1f%  Time: %g ms\n", n, integral, (sphere.exact- integral)/sphere.exact,  (max-min)/max*100, stepms);
    }
#endif
    ...
    for (uint n=1; n<N; n<<=1) {
       ...
       printf("IcosaVoronoi, n = %5d, I= %15.10g (%7.4f %%),  dVe= %4.1f%  Time: %g ms\n", n, integral, (sphere.exact- integral)/sphere.exact,  (max-min)/max*100, stepms);
    }
#endif
    ...
    for (uint n=1; n<N; n<<=1) {
       t1 = std::chrono::high_resolution_clock::now();
       IcosahedronSquare sphere(n,n);
       real integral = sphere.integral();
       real stepms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now()-t1).count();
       printf("IcosaSquare, n = %5d, I= %15.10g (%7.4f %%)   Time: %g ms\n", n, integral, (sphere.exact-integral)/sphere.exact, stepms);
    }
#endif
    return 0;
}
