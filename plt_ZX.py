...
def readmeta(fn):
    ...
    vy_grid = [ float(x.strip()) for x in f.readline().split() ]
    vz_grid = [ float(x.strip()) for x in f.readline().split() ]
    dz = (float(z1)-float(z0))/int(nz)

    return int(nx), int(nz), int(sv), x_grid, z_grid, vx_grid, vy_grid, vz_grid, dz

meta = glob.glob("*.meta")[0]
nx, nz, nv, xc, zc, vx_grid, vy_grid, vz_grid, dz = readmeta(meta)
...
for fname in files:
    ...
    for v in [0,1,2]:
        ...
        #ax[0,v].set_aspect(1)
        ax[0,v].set_xlabel("Z@v=({} {} {})".format(vx_grid[v], vy_grid[v], vz_grid[v] ))
