//  Adapted from https://github.com/bernatguillen/dG_project
...
void LSWInv(std::vector<double> &w, std::vector<std::vector<double>> &M){
	int n = w.size()+1;
	gsl_matrix *W     = gsl_matrix_alloc(n,n);
	gsl_matrix *W_inv = gsl_matrix_alloc(n,n);
	gsl_permutation *perm = gsl_permutation_alloc(n);
	for(int i = 0; i<n-1; ++i){
	    gsl_matrix_set(W, i, n-1,   w[i]);
	    gsl_matrix_set(W, n-1, i,   w[i]);
	    gsl_matrix_set(W, i,   i, 2*w[i]);
	}
	int s;
	gsl_linalg_LU_decomp(W, perm, &s);
	gsl_linalg_LU_invert(W, perm, W_inv);
	for(int i =0; i<n; ++i)
	for(int j =0; j<n; ++j){
	    M[i][j] = gsl_matrix_get(W_inv, i,j);
	}
	gsl_matrix_free(W);
